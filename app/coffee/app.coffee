...
@tuesmon = tuesmon = {}
tuesmon.emojis = window.emojis
@.tuesmonContribPlugins = @.tuesmonContribPlugins or window.tuesmonContribPlugins or []

# Generic function for generate hash from a arbitrary length
# collection of parameters.
tuesmon.generateHash = (components=[]) ->
    components = _.map(components, (x) -> JSON.stringify(x))
    return hex_sha1(components.join(":"))


tuesmon.generateUniqueSessionIdentifier = ->
    date = (new Date()).getTime()
    randomNumber = Math.floor(Math.random() * 0x9000000)
    return tuesmon.generateHash([date, randomNumber])


tuesmon.sessionId = tuesmon.generateUniqueSessionIdentifier()
...
configure = ($routeProvider, $locationProvider, $httpProvider, $provide, $tgEventsProvider,
    ...
    defaultHeaders = {
        ...
        "Accept-Language": window.tuesmonConfig.defaultLanguage || "en"
        "X-Session-Id": tuesmon.sessionId
    }
    ...
    $tgEventsProvider.setSessionId(tuesmon.sessionId)
    ...
    $compileProvider.debugInfoEnabled(window.tuesmonConfig.debugInfo || false)

    if localStorage.userInfo
        userInfo = JSON.parse(localStorage.userInfo)

    # i18n
    preferedLangCode = userInfo?.lang || window.tuesmonConfig.defaultLanguage || "en"

    $translatePartialLoaderProvider.addPart('tuesmon')
    $translateProvider
...
init = ($log, $rootscope, $auth, $events, $analytics, $translate, $location, $navUrls, appMetaService,
    ...
    validators = {
        linewidth: (val, width) ->
            lines = tuesmon.nl2br(val).split("<br />")
    ...
    # Tuesmon Plugins
    $rootscope.contribPlugins = @.tuesmonContribPlugins
    $rootscope.adminPlugins = _.filter(@.tuesmonContribPlugins, {"type": "admin"})
    $rootscope.userSettingsPlugins = _.filter(@.tuesmonContribPlugins, {"type": "userSettings"})
...
# Load modules
pluginsWithModule = _.filter(@.tuesmonContribPlugins, (plugin) -> plugin.module)
