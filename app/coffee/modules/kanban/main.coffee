...
mixOf = @.tuesmon.mixOf
toggleText = @.tuesmon.toggleText
scopeDefer = @.tuesmon.scopeDefer
bindOnce = @.tuesmon.bindOnce
groupBy = @.tuesmon.groupBy
timeout = @.tuesmon.timeout
bindMethods = @.tuesmon.bindMethods
debounceLeading = @.tuesmon.debounceLeading

module = angular.module("tuesmonKanban")

#############################################################################
## Kanban Controller
#############################################################################

class KanbanController extends mixOf(tuesmon.Controller, tuesmon.PageMixin, tuesmon.FiltersMixin, tuesmon.UsFiltersMixin)
    @.$inject = [
        "$scope",
        "$rootScope",
        "$tgRepo",
    ...
    constructor: (@scope, @rootscope, @repo, @confirm, @rs, @rs2, @params, @q, @location,
        ...
        tuesmon.defineImmutableProperty @.scope, "usByStatus", () =>
    ...
    initializeSubscription: ->
        ...
        randomTimeout = tuesmon.randomInt(700, 1000)
    ...
    moveUs: (ctx, usList, newStatusId, index) ->
        ...
        promise.then () =>
            ...
            promise.then (result) =>
                ...
                if headers && headers['tuesmon-info-order-updated']
                    order = JSON.parse(headers['tuesmon-info-order-updated'])
