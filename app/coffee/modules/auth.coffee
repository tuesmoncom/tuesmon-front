...
module = angular.module("tuesmonAuth", ["tuesmonResources"])
...
class AuthService extends tuesmon.Service
    @.$inject = ["$rootScope",
                 "$tgStorage",
                 "$tgModel",
                 "$tgResources",
    ...
    _getUserTheme: ->
        return @rootscope.user?.theme || @config.get("defaultTheme") || "tuesmon" # load on index.jade
