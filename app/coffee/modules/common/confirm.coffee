...
cancelTimeout = @.tuesmon.cancelTimeout
...
class ConfirmService extends tuesmon.Service
    ...
    success: (title, message, icon, action) ->
        ...
        if icon
            if icon.type == "img"
                ...
                tuesmon.addClass(detailImage, "icon")
                tuesmon.addClass(detailImage, icon.name)
                tuesmon.addClass(detailImage, "lb-icon")
