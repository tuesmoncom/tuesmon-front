...
class EmojisService extends tuesmon.Service
    @.$inject = []

    constructor: () ->
        @.emojis = _.map tuesmon.emojis, (it) ->
    ...
    replaceImgsByEmojiName: (html) =>
        emojiIds = tuesmon.getMatches(html, /emojis\/([^"]+).png"/gi)
    ...
    replaceEmojiNameByImgs: (text) =>
        emojiIds = tuesmon.getMatches(text, /:([\w +-]*):/g)
