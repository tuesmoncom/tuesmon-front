...
startswith = @.tuesmon.startswith
bindMethods = @.tuesmon.bindMethods
debounceLeading = @.tuesmon.debounceLeading

module = angular.module("tuesmonIssues")

#############################################################################
## Issues Controller
#############################################################################

class IssuesController extends mixOf(tuesmon.Controller, tuesmon.PageMixin, tuesmon.FiltersMixin)
    @.$inject = [
        "$scope",
        "$rootScope",
        "$tgRepo",
...
IssueStatusInlineEditionDirective = ($repo, $template, $rootscope) ->
    ...
    link = ($scope, $el, $attrs) ->
        ...
        tuesmon.bindOnce $scope, "project", (project) ->
