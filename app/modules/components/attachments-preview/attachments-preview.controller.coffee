...
class AttachmentsPreviewController
    ...
    constructor: (@attachmentsPreviewService) ->
        tuesmon.defineImmutableProperty @, "current", () =>
    ...
    hasPagination: () ->
        images = @.attachments.filter (attachment) =>
            return tuesmon.isImage(attachment.getIn(['file', 'name']))
...
angular.module('tuesmonComponents').controller('AttachmentsPreview', AttachmentsPreviewController)
