...
class WysiwygMentionService
    ...
    searchItem: (term) ->
        return new Promise (resolve, reject) =>
            term = tuesmon.slugify(term)

            filter = (item) ->
                return ['subject', 'ref'].some((attr) ->
                    tuesmon.slugify(item[attr]).indexOf(term) >= 0
                )
    ...
    searchUser: (term, callback) ->
        users = @projectService.project.toJS().members.filter (user) ->
            return ['username', 'full_name', 'full_name_display'].some((attr) ->
                tuesmon.slugify(user[attr]).indexOf(term) >= 0 || user[attr].indexOf(term) >= 0
            )
...
angular.module("tuesmonComponents").service("tgWysiwygMentionService", WysiwygMentionService)
