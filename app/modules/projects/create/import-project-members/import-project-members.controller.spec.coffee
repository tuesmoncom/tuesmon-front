...
describe "ImportProjectMembersCtrl", ->
    ...
    it "confirm user", () ->
        ...
        ctrl.confirmUser('user', 'tuesmon-user')

        expect(ctrl.selectedUsers.size).to.be.equal(1)

        expect(ctrl.selectedUsers.get(0).get('user')).to.be.equal('user')
        expect(ctrl.selectedUsers.get(0).get('tuesmonUser')).to.be.equal('tuesmon-user')
    ...
    it "check users limit", () ->
     ...
     it "get distict select tuesmon users excluding the current user", () ->
        ctrl = $controller("ImportProjectMembersCtrl")
        ctrl.selectedUsers = Immutable.fromJS([
            {
                tuesmonUser: {
        ...
        users = ctrl.getDistinctSelectedTuesmonUsers()
     ...
     it "refresh selectable users array with the selected ones", () ->
        ...
        ctrl.getDistinctSelectedTuesmonUsers = sinon.stub().returns(Immutable.fromJS([
            {tuesmonUser: 1}
        ]))
        ctrl.displayEmailSelector = false
    ...
    it "import more user disable in private project", () ->
        ...
        ctrl.getDistinctSelectedTuesmonUsers = sinon.stub().returns(Immutable.fromJS([1,2,3]))
